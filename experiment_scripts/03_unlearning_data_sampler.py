...
for sample_size in RANDOM_SAMPLE_UNLEARNING_SIZES:
    for i in range(REPETITIONS_OF_EACH_SAMPLE_SIZE):
        os.makedirs(f"experiments/{DATASET_NAME}/unlearning/sample_size_{sample_size}/sample_{i}/data/", exist_ok=True)
...
training_dataset = torch.load(f"experiments/{DATASET_NAME}/splits/{DATASET_NAME}_train.pt")

# data format is a list of tuples (sequence_list, user_id)
for user_id in random_user_ids:
    logging.info(f"Sampling user {user_id}")
    os.makedirs(f'experiments/{DATASET_NAME}/unlearning/user_sample/user_{user_id}/data', exist_ok=True)
    unlearning_indexes = [i for i, (_, u_id) in enumerate(training_dataset) if u_id == user_id]
    remaining_indexes = [i for i, (_, u_id) in enumerate(training_dataset) if u_id != user_id]
    torch.save(unlearning_indexes, f"experiments/{DATASET_NAME}/unlearning/user_sample/user_{user_id}/data/unlearning.indexes.pt")
    torch.save(remaining_indexes, f"experiments/{DATASET_NAME}/unlearning/user_sample/user_{user_id}/data/remaining.indexes.pt")
