...
for i in range(REPETITIONS_OF_EACH_SAMPLE_SIZE):
    ...
    wandb.init(
        project="Thesis",
        job_type="baseline",
        name=f"{"NegGradPlus" if NEG_GRAD_PLUS else "NegGrad"}-{DATASET_NAME}-{MODEL_NAME}-{SCENARIO}_unlearning-sample_size_{SAMPLE_SIZE}-repetition_{i}",
        config={
    ...
    optimizer = AdamW(model.parameters(), lr=NEG_GRAD_LEARNING_RATE)
    unlearning_stats = {}
    ...
    for unlearning_epoch in range(NUMBER_OF_EPOCHS):
        ...
        for unlearning_batch, remaining_batch in pbar:
            ...
            if NEG_GRAD_PLUS:
                model_output_remaining = model(**inputs_remaining)
                model_output_unlearning = model(**inputs_unlearning)
                loss = F.cross_entropy(model_output_remaining.logits, labels_remaining) - F.cross_entropy(model_output_unlearning.logits, labels_unlearning)
                loss.backward()
                optimizer.step()
                total_epoch_loss += loss.item()
            else:
                model_output_unlearning = model(**inputs_unlearning)
                loss = -1 * F.cross_entropy(model_output_unlearning.logits, labels_unlearning)
                loss.backward()
                optimizer.step()
                total_epoch_loss += loss.item()
        ...
        unlearning_eval_results = compute_metrics(get_model_outputs(model, unlearning_dloader, device))
        test_eval_results = compute_metrics(get_model_outputs(model, test_dloader, device))
