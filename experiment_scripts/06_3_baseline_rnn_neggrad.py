...
for i in range(REPETITIONS_OF_EACH_SAMPLE_SIZE):
    ...
    remaining_dloader = DataLoader(remaining_dataset, batch_size=SAMPLE_SIZE, collate_fn=custom_collate_fn, shuffle=True, num_workers=24)
    unlearning_dloader = DataLoader(unlearning_dataset, batch_size=SAMPLE_SIZE, collate_fn=custom_collate_fn, shuffle=True, num_workers=24)
    ...
    for unlearning_epoch in range(NUMBER_OF_EPOCHS):
        ...
        unlearning_data_eval = model.test_model(unlearning_dloader)
        test_data_eval = model.test_model(test_dloader)
