...
failed_commands_log="$log_dir/eval_tf_cmds_${timestamp}_failed.txt"
> "$failed_commands_log"
                ...
                for method in "${methods[@]}"; do
                    if [[ "$method" == "trace_hiding" ]]; then
                        ...
                        for importance in "${importances[@]}"; do
                            cmd="python experiment_scripts/07_1_metrics_eval_transformer.py --model $model --dataset $dataset --scenario user --method $method --sampleSize $sampleSize --biased $bias --batchSize 20 --importance $importance"
                            echo "$cmd" >> "$command_file"
                        done
                    else
                        # Otherwise, exclude the importance argument
                        cmd="python experiment_scripts/07_1_metrics_eval_transformer.py --model $model --dataset $dataset --scenario user --method $method --sampleSize $sampleSize --biased $bias --batchSize 20"
                        echo "$cmd" >> "$command_file"
                    fi
                done
            done
