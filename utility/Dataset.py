...
class HexagonTrajectoryUserDataset(Dataset):
    def __init__(self, csv_file, trajectory_col_name="higher_order_trajectory", user_id_col_name='user_id', cell_to_id=None, min_trajectory_length=5, min_num_trajectories=2, split_ratio=0.8, random_state=42):
        ...
        Args:
            ...
            min_trajectory_length (int, optional): The minimum length of a trajectory. Defaults to 5. Only trajectories with length >= min_trajectory_length will be considered.
            min_num_trajectories (int, optional): The minimum number of trajectories a user should have. Defaults to 2. Only users with number of trajectories >= min_num_trajectories will be considered.
        ...
        # Delete trajectories that are shorter than min_trajectory_length
        self.data = self.data[self.data[trajectory_col_name].apply(
            lambda x: len(x.split(sep=' ')) >= min_trajectory_length)]

        # Delete users who have fewer than min_num_trajectories
        self.data = self.data.groupby(user_id_col_name).filter(
            lambda x: len(x) >= min_num_trajectories)

        # Get all cell names
        traj_values = self.data[trajectory_col_name]
        cell_names = set()
        for trajectory in traj_values:
            for cell_name in trajectory.split(sep=' '):
        ...
        self.vocal_size = len(cell_names)
        ...
        self.data[self.TRAJECTORY_COLUMN] = self.data[trajectory_col_name].apply(
            lambda traj: [self.cell_to_id[cell_name] for cell_name in traj.split(sep=' ')])
